...
export const computeAnnualizedReturnPercent = (series) => {
  ...
  return (Math.pow(series[series.length - 1].value / series[0].value, 1 / years) - 1) * 100;
};
...
export const computeStatsFromSeries = (series) => {
  ...
  return {
    ...
    // "תשואה מאז תחילת ההשקעה" - the return since inception, read straight
    // off the dynamic performance curve (requirement: derive it from the
    // same mechanism, not from a separately maintained number).
    totalReturnPercent: computeTotalReturnPercent(points),
    annualizedReturnPercent: computeAnnualizedReturnPercent(points),
...
export const computePortfolioStats = (snapshots) => computeStatsFromSeries(buildEquitySeries(snapshots));
